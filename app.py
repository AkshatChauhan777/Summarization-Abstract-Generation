...
def extract_text_from_file(uploaded_file, ext):
    ...
    if ext == ".txt":
        ...
        try:
            data = json.load(uploaded_file)
            text = " ".join(data.values()) if isinstance(data, dict) else json.dumps(data)
        except Exception as e:
            st.error(f"Error parsing JSON: {e}")
            return ""
    else:
...
def highlight_keywords(text, keywords):
    ...
    for kw in sorted(keyword_list, key=len, reverse=True):
        ...
        text = re.sub(fr"\b({escaped})\b", r"**\1**", text, flags=re.IGNORECASE)
    return text
...
# build a HF pipeline instance for summarization
summarizer = pipeline(
    "summarization",
...
def summarize_text(text, length_ratio, highlight=True):
    ...
    if orig_lang != "en":
        text = translate_text(text, orig_lang, "en")
        translated = True

    wc = len(text.split())
    min_len = max(10, int(wc * length_ratio * 0.8))
    max_len = int(wc * length_ratio * 1.2)
    ...
    summary_text = ensure_complete_sentences(result[0]["summary_text"])

    if not highlight:
        if translated:
            summary_text = translate_text(summary_text, "en", orig_lang)
        return summary_text, [], len(summary_text.split())

    # highlight mode
    keywords = extract_keywords_yake(summary_text)
    if translated:
        # bring keywords back to original language
        kw_orig = []
        for kw, score in keywords:
            try:
                kw0 = translate_text(kw, "en", orig_lang)
            except:
                kw0 = kw
            kw_orig.append((kw0, score))
        keywords = kw_orig

    highlighted = highlight_keywords(summary_text, keywords)
    if translated:
        highlighted = translate_text(highlighted, "en", orig_lang)

    return highlighted, keywords, len(summary_text.split())
...
def generate_abstract(text, max_input_length=1024, max_output_length=400):
    ...
    out_ids = abstract_model.generate(
        input_ids=inputs.input_ids,
        attention_mask=inputs.attention_mask,
        max_length=max_output_length,
        min_length=250,
    ...
    return abstract_tokenizer.decode(out_ids[0], skip_special_tokens=True)
...
if task == "Generate Summary":
    method = st.sidebar.radio("Manual text entry?", ("Yes", "No"))
    text = ""
    ...
    if method == "Yes":
        ...
        choose = st.sidebar.radio("Choose Upload Type", ("Upload File", "Enter URL"))
        if choose == "Upload File":
            f = st.sidebar.file_uploader(
                "Upload document",
                type=["txt","pdf","docx","xlsx","csv","pptx","html","odt","epub","xml","json"]
            )
            if f:
                ext = os.path.splitext(f.name)[1].lower()
                try:
                    text = extract_text_from_file(f, ext)
                    st.success(f"✅ Text extracted from `{f.name}`")
                    st.markdown(f"**🧾 Word Count:** {len(text.split())}")
                except Exception as e:
                    st.error(f"❌ Failed to extract text: {e}")
        else:
    ...
    if text:
        ...
        summary_type = st.sidebar.selectbox(
            "Choose summary type",
            ["Concise","Brief","Moderate","Detailed"]
        )
        ratios = {
            "Concise": 0.25,
            "Brief":   0.4,
        ...
        if st.sidebar.button("Generate Summary"):
            summary, keywords, wc = summarize_text(text, ratios[summary_type], highlight=True)
            st.markdown(f"### 📝 {summary_type} Summary")
            st.markdown(summary)
            st.markdown(f"**🧾 Summary Word Count:** {wc}")
            st.markdown("### 🔑 Keywords")
            for kw, _ in keywords:
                st.markdown(f"- {kw}")

elif task == "Generate Abstract":
    inp = st.text_area("📝 Enter text for abstract generation:", height=300)
    if inp and st.button("Generate Abstract"):
        with st.spinner("Generating abstract..."):
            abs_text = generate_abstract(inp)
            st.markdown("### 📚 Generated Abstract")
            st.markdown(abs_text)
            st.markdown(f"**🧾 Abstract Word Count:** {len(abs_text.split())}")
